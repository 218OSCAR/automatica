// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from zivid_interfaces:srv/CameraInfoModelName.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "zivid_interfaces/srv/detail/camera_info_model_name__struct.h"
#include "zivid_interfaces/srv/detail/camera_info_model_name__type_support.h"
#include "zivid_interfaces/srv/detail/camera_info_model_name__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace zivid_interfaces
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _CameraInfoModelName_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _CameraInfoModelName_Request_type_support_ids_t;

static const _CameraInfoModelName_Request_type_support_ids_t _CameraInfoModelName_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _CameraInfoModelName_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _CameraInfoModelName_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _CameraInfoModelName_Request_type_support_symbol_names_t _CameraInfoModelName_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, zivid_interfaces, srv, CameraInfoModelName_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, zivid_interfaces, srv, CameraInfoModelName_Request)),
  }
};

typedef struct _CameraInfoModelName_Request_type_support_data_t
{
  void * data[2];
} _CameraInfoModelName_Request_type_support_data_t;

static _CameraInfoModelName_Request_type_support_data_t _CameraInfoModelName_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _CameraInfoModelName_Request_message_typesupport_map = {
  2,
  "zivid_interfaces",
  &_CameraInfoModelName_Request_message_typesupport_ids.typesupport_identifier[0],
  &_CameraInfoModelName_Request_message_typesupport_symbol_names.symbol_name[0],
  &_CameraInfoModelName_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t CameraInfoModelName_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_CameraInfoModelName_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &zivid_interfaces__srv__CameraInfoModelName_Request__get_type_hash,
  &zivid_interfaces__srv__CameraInfoModelName_Request__get_type_description,
  &zivid_interfaces__srv__CameraInfoModelName_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace zivid_interfaces

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, zivid_interfaces, srv, CameraInfoModelName_Request)() {
  return &::zivid_interfaces::srv::rosidl_typesupport_c::CameraInfoModelName_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__struct.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__type_support.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace zivid_interfaces
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _CameraInfoModelName_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _CameraInfoModelName_Response_type_support_ids_t;

static const _CameraInfoModelName_Response_type_support_ids_t _CameraInfoModelName_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _CameraInfoModelName_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _CameraInfoModelName_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _CameraInfoModelName_Response_type_support_symbol_names_t _CameraInfoModelName_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, zivid_interfaces, srv, CameraInfoModelName_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, zivid_interfaces, srv, CameraInfoModelName_Response)),
  }
};

typedef struct _CameraInfoModelName_Response_type_support_data_t
{
  void * data[2];
} _CameraInfoModelName_Response_type_support_data_t;

static _CameraInfoModelName_Response_type_support_data_t _CameraInfoModelName_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _CameraInfoModelName_Response_message_typesupport_map = {
  2,
  "zivid_interfaces",
  &_CameraInfoModelName_Response_message_typesupport_ids.typesupport_identifier[0],
  &_CameraInfoModelName_Response_message_typesupport_symbol_names.symbol_name[0],
  &_CameraInfoModelName_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t CameraInfoModelName_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_CameraInfoModelName_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &zivid_interfaces__srv__CameraInfoModelName_Response__get_type_hash,
  &zivid_interfaces__srv__CameraInfoModelName_Response__get_type_description,
  &zivid_interfaces__srv__CameraInfoModelName_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace zivid_interfaces

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, zivid_interfaces, srv, CameraInfoModelName_Response)() {
  return &::zivid_interfaces::srv::rosidl_typesupport_c::CameraInfoModelName_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__struct.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__type_support.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace zivid_interfaces
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _CameraInfoModelName_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _CameraInfoModelName_Event_type_support_ids_t;

static const _CameraInfoModelName_Event_type_support_ids_t _CameraInfoModelName_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _CameraInfoModelName_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _CameraInfoModelName_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _CameraInfoModelName_Event_type_support_symbol_names_t _CameraInfoModelName_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, zivid_interfaces, srv, CameraInfoModelName_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, zivid_interfaces, srv, CameraInfoModelName_Event)),
  }
};

typedef struct _CameraInfoModelName_Event_type_support_data_t
{
  void * data[2];
} _CameraInfoModelName_Event_type_support_data_t;

static _CameraInfoModelName_Event_type_support_data_t _CameraInfoModelName_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _CameraInfoModelName_Event_message_typesupport_map = {
  2,
  "zivid_interfaces",
  &_CameraInfoModelName_Event_message_typesupport_ids.typesupport_identifier[0],
  &_CameraInfoModelName_Event_message_typesupport_symbol_names.symbol_name[0],
  &_CameraInfoModelName_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t CameraInfoModelName_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_CameraInfoModelName_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &zivid_interfaces__srv__CameraInfoModelName_Event__get_type_hash,
  &zivid_interfaces__srv__CameraInfoModelName_Event__get_type_description,
  &zivid_interfaces__srv__CameraInfoModelName_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace zivid_interfaces

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, zivid_interfaces, srv, CameraInfoModelName_Event)() {
  return &::zivid_interfaces::srv::rosidl_typesupport_c::CameraInfoModelName_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "zivid_interfaces/srv/detail/camera_info_model_name__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace zivid_interfaces
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _CameraInfoModelName_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _CameraInfoModelName_type_support_ids_t;

static const _CameraInfoModelName_type_support_ids_t _CameraInfoModelName_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _CameraInfoModelName_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _CameraInfoModelName_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _CameraInfoModelName_type_support_symbol_names_t _CameraInfoModelName_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, zivid_interfaces, srv, CameraInfoModelName)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, zivid_interfaces, srv, CameraInfoModelName)),
  }
};

typedef struct _CameraInfoModelName_type_support_data_t
{
  void * data[2];
} _CameraInfoModelName_type_support_data_t;

static _CameraInfoModelName_type_support_data_t _CameraInfoModelName_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _CameraInfoModelName_service_typesupport_map = {
  2,
  "zivid_interfaces",
  &_CameraInfoModelName_service_typesupport_ids.typesupport_identifier[0],
  &_CameraInfoModelName_service_typesupport_symbol_names.symbol_name[0],
  &_CameraInfoModelName_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t CameraInfoModelName_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_CameraInfoModelName_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &CameraInfoModelName_Request_message_type_support_handle,
  &CameraInfoModelName_Response_message_type_support_handle,
  &CameraInfoModelName_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    zivid_interfaces,
    srv,
    CameraInfoModelName
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    zivid_interfaces,
    srv,
    CameraInfoModelName
  ),
  &zivid_interfaces__srv__CameraInfoModelName__get_type_hash,
  &zivid_interfaces__srv__CameraInfoModelName__get_type_description,
  &zivid_interfaces__srv__CameraInfoModelName__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace zivid_interfaces

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, zivid_interfaces, srv, CameraInfoModelName)() {
  return &::zivid_interfaces::srv::rosidl_typesupport_c::CameraInfoModelName_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
