## this file is taken from https://gitlab.com/botasys/python_interface
## the original file is called bota_ethercat_minimal_example.py
## this file is slightly adjusted
...
class BotaSensOne:
    ...
    def __init__(self, ifname, child_conn):
        self._ifname = ifname
    ...
    def bota_sensor_setup(self, slave_pos):
        ...
        print("BOTA Sampling rate {}".format(sampling_rate))
        if sampling_rate > 0:
            self.time_step = 1.0/float(sampling_rate)

        print("BOTA time step {}".format(self.time_step))
    ...
    def run(self):
        ...
        if self._master.config_init() > 0:
            ...
            print("BOTA {} slaves found and configured".format(
            ...
            if self._master.state_check(pysoem.SAFEOP_STATE, 50000) != pysoem.SAFEOP_STATE:
                ...
                for slave in self._master.slaves:
                    if not slave.state == pysoem.SAFEOP_STATE:
                        print('BOTA {} did not reach SAFEOP state'.format(slave.name))
                        print('al status code {} ({})'.format(hex(slave.al_status),
                                                              pysoem.al_status_code_to_string(slave.al_status)))
                raise Exception('BOTA not all slaves reached SAFEOP state')
            ...
            if self._master.state != pysoem.OP_STATE:
                ...
                for slave in self._master.slaves:
                    if not slave.state == pysoem.OP_STATE:
                        print('BOTA {} did not reach OP state'.format(slave.name))
                        print('al status code {} ({})'.format(hex(slave.al_status),
                                                              pysoem.al_status_code_to_string(slave.al_status)))
            ...
            try:
                while 1:
                    ...
                    # send to other script with Pipe
                    msg = [Fx, Fy, Fz]
            ...
            print('BOTA slaves not found')


def get_data(child_conn):
    try:
        BotaSensOne(cfg.ADAPTER, child_conn).run()  # adapter can be found with find_ethernet_adapters.py, see https://gitlab.com/botasys/python_interface
