...
def imshow_det_bboxes(img,
    ...
    ### UNDER CONSTRUCTION #######################################################################################################################
      # print('\n\n')
      # print(len(segms))         ## apparently there's a limit of max. 100 segments
      # print()
      # print(labels)             ## somehow always shoes zero (=cable)
      # print(class_names)        ## tuple of 3 possible labels ('cable','HV','pneumatic'), will be indexed via class_names[labels[i]]
      # print('\n\n')
        ...
        #print(type(pols_json))
        #print(len(pols_json))
        #print(labels)
        #print(labels[num_bboxes:])
        #print(scores)
        #Add label and score to polygons:
        json_full = {}
        for key,poly in pols_json.items():            
            #print(str(key)+":")
            json_full[key] = {'label': class_names[labels[int(key)]] , 'score': float(scores[int(key)]) , 'data': poly}
        json_object = json.dumps(json_full, indent=3) 
        
        import config as cfg
        json_path = cfg.PATHS['solo_polygons']
...
def imshow_gt_det_bboxes(img,
    ...
    """
    print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXStart"+str(out_file))
    ...
    if overlay_gt_pred:
        ...
        if show:
            if wait_time == 0:
                ...
                plt.show(block=False)
                plt.pause(wait_time)
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"+str(out_file))
        if out_file is not None:
            mmcv.imwrite(img, out_file)
